                            ...
                            <div class="table-responsive">
                                <table id="table" class="table table-hover table-bordered">
                                    ...
                                    @foreach( $meetings as $meeting )
                                        <tr class="cat{{$meeting->id}}">

                                            <td>{{$meeting->id}}</td>
                                            <td>{{$meeting->topic}}</td>
                                            <td>{{$meeting->representative->name}} ({{$meeting->representative->position}})</td>
                                            <td>{{$meeting->period_admin->start_date}} - {{$meeting->period_admin->end_date}}</td>
                                            <td width="10px">
                                                <a href="{{ route('reunion.edit', $meeting->id) }}" class=" btn btn-info btn-sm">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                            </td>
                                            <td width="10px">
                                                <a href="{{ route('reunion.edit', $meeting->id) }}" class="edit-modal-cat btn btn-warning btn-sm">
                                                    <i class="fa fa-pencil"></i>
                                                </a>
                                            </td>
                                            <td width="10px">
                                                {!! Form::open(['route' => ['reunion.destroy', $meeting->id], 'method' => 'DELETE']) !!}
